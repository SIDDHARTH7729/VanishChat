...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-30 bg-black/30 backdrop-blur-md border-b border-cyan-500/30 shadow-lg">
      <div className="max-w-7xl mx-auto px-6 sm:px-8">
        <div className="flex items-center justify-between h-20">
          
          <div className="flex-1 md:flex-initial">
            <Link href="/" className="block md:inline-block">
              <Image src="/logo(3).png" alt="Logo" width={90} height={90} />
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <Link
                key={item.path}
                href={item.path}
                className="relative text-cyan-400 px-4 py-2 text-sm cursor-pointer font-medium transition-all duration-300 hover:text-purple-600"
              >
